...
import { FeaturedOnlineOffersCarousel } from '../FeaturedOnlineOffersCarousel';
...
describe('FeaturedOnlineOffersGrid', () => {
  ...
  it('should render correctly', async () => {
    const { getByText } = render(<FeaturedOnlineOffersCarousel onPress={() => {}} />);
  ...
  it('should render correctly when loading', async () => {
    ...
    const { getAllByTestId } = render(<FeaturedOnlineOffersCarousel onPress={() => {}} />);
    expect(getAllByTestId('product-item-skeleton').length).toEqual(3);
  });

  it('should handle click on carousel', async () => {
    const mockOnPress = jest.fn();
    const { getByTestId } = render(<FeaturedOnlineOffersCarousel onPress={mockOnPress} testID="featured-offers" />);
  ...
  it('should display at most 6 featured offers item', async () => {
    const { getAllByText } = render(
      <FeaturedOnlineOffersCarousel numberOfItems={6} onPress={() => {}} testID="featured-offers" />
    );
